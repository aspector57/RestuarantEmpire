...
namespace RestaurantEmpire.Sim
{
    ...
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            // so the harness no longer estimates a labour figure at all.
            var wage = Dec(args, "--wage", 18m);
            ...
            for (var i = 0; i < Int(args, "--servers", 1); i++)
                restaurant.Payroll.Hire(new Employee("server-" + (i + 1), "Server " + (i + 1), StaffRole.Server, wage * 0.8m, 0.5m));
            ...
            var session = new Session(runner, company,
                labourPerDay: 0m,   // the simulation bills wages itself now
                ...
                while (true)
                {
                    ...
                    switch (input.Trim())
                    {
                        ...
                        case "7":
                            ...
                            var rate = hiringCook ? 18m : 14.40m;
...
the question it exists to answer is whether being stopped feels worth it.
  ...
  --wage <n>             cook hourly rate; servers earn 80%  (default 18)
