...
namespace RestaurantEmpire.Sim
{
    ...
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            // and is exactly how you pay a morning's labor for an empty room.
            ...
            // A starting crew. Wages are now GENERATED by the simulation from this payroll,
            // so the harness no longer estimates a labor figure at all.
            var wage = Dec(args, "--wage", 16m);
            for (var i = 0; i < Int(args, "--cooks", 1); i++)
                restaurant.Payroll.Hire(new Employee("cook-" + (i + 1), "Cook " + (i + 1), StaffRole.Cook, wage, 0.5m));
            for (var i = 0; i < Int(args, "--servers", 1); i++)
                restaurant.Payroll.Hire(new Employee("server-" + (i + 1), "Server " + (i + 1), StaffRole.Server, wage * 0.75m, 0.5m));
            ...
            var session = new Session(runner, company,
                laborPerDay: 0m,   // the simulation bills wages itself now
        ...
        private sealed class Session
        {
            ...
            private readonly decimal _laborPerDay, _overheadPerDay;
            ...
            public Session(SimulationRunner runner, Company company, decimal laborPerDay, decimal overheadPerDay)
            {
                _runner = runner;
                _company = company;
                _laborPerDay = laborPerDay;
            ...
            private void BookADay()
            {
                ...
                if (_laborPerDay > 0m) _company.Economy.Record(tick, LedgerCategory.LaborCost, _laborPerDay, "Brigade", id);
            ...
            private bool Act()
            {
                ...
                while (true)
                {
                    ...
                    switch (input.Trim())
                    {
                        ...
                        case "7":
                            Console.Write("    hire a [c]ook or a [s]erver? > ");
                            var hireWhat = Console.ReadLine();
                            if (hireWhat == null) return false;

                            var hiringCook = hireWhat.Trim().ToLowerInvariant().StartsWith("c");
                            var role = hiringCook ? StaffRole.Cook : StaffRole.Server;
                            var rate = hiringCook ? 16m : 12m;
                            var number = restaurant.Payroll.CountOf(role) + 1;

                            restaurant.Payroll.Hire(new Employee(
                                role.ToString().ToLowerInvariant() + "-" + Guid.NewGuid().ToString("N").Substring(0, 6),
                                role + " " + number, role, rate, 0.5m));
                        ...
                        case "8":
                            Console.Write("    let go a [c]ook or a [s]erver? > ");
                            var fireWhat = Console.ReadLine();
                            if (fireWhat == null) return false;

                            var firedRole = fireWhat.Trim().ToLowerInvariant().StartsWith("c") ? StaffRole.Cook : StaffRole.Server;
                            var gone = restaurant.Payroll.FireOne(firedRole);

                            if (gone == null) { Console.WriteLine("    nobody in that role to let go."); break; }
        ...
        /// </summary>
        private static Neighborhood ChooseASite()
        {
            var sites = new[]
            {
                Neighborhood.CityCenter(),
                Neighborhood.BusinessDistrict(),
                Neighborhood.NightlifeQuarter(),
                Neighborhood.SuburbanHighStreet()
            };
        ...
        private static Neighborhood Where(string key)
        {
...
the question it exists to answer is whether being stopped feels worth it.
  ...
  --cooks <n>            cooks on the payroll                (default 1)
  --servers <n>          servers on the payroll              (default 1)
  --wage <n>             cook hourly rate; servers earn 75%  (default 16)
