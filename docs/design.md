...
**Public moments — awards, interviews, and how you handle them (added per Aaron).** Marketing's review-response lever above is currently a single flat switch: respond or don't, one fixed payoff. The same shape generalizes to a more interesting cousin — an award nomination, an interview request, a chance to speak at a ceremony. These arrive as an accept/decline moment, reusing Marquee Opportunity's bid/pass shell (declining is sometimes correctly the right call, not a wasted invite) — but the real axis is HOW you respond, not whether. A small, reusable set of tones rather than a slider: gracious/humble (low variance, small upside, near-zero downside — the safe default); confident/self-promotional (biggest upside, but it borrows against Reputation not yet earned — the identical overclaim-punishment mechanism M2 validated with ingredient quality and price-gouging: talk that outruns the food gets clawed back hard once guests arrive expecting it); and combative/defensive (clapping back at a bad review from the podium, needling a named rival on stage — lands well with part of the audience, but costs standing with that specific Competitor, reusing the friendliness relationship that system already tracks). None of this needs new architecture: accept/decline is Marquee's shell, the tones are a small enum on the existing review-response lever, and the relationship cost reuses Competitors' existing field.

**Where the payoff lands is the second idea this generates, not a separate one.** A Restaurant's Reputation is explicitly a property of the building (Phase 4: "the container for a single physical location") — but an interview or an award nomination happens to a chef, not a building, and it would be wrong for that payoff to vanish entirely into whichever restaurant happens to be flagship at the time. Some of it should land on the specific restaurant being discussed, as today; the more durable part belongs somewhere new.

**Personal Chef Prestige — a score that belongs to you, not the building (added per Aaron).** Every Reputation score built so far lives on a Restaurant — earned there, read there, decays there. That's correct for a single restaurant, but it leaves a real gap once a second location exists: does restaurant #2 open at zero Reputation, as if the person who built #1 into something respected had never cooked a day in their life? Real multi-location openings don't work that way — a chef's name buys a new location genuine buzz on day one, before a single plate has gone out.

So: a second score, held at the **Company** level (the "you" the design already treats as one entity — see Company/Empire, Phase 9), fed by signature-dish creation (already personal and non-delegable per Binding Principle 1), by the public-moments mechanic above, and by the *sustained* Reputation of restaurants carrying your name — not their instantaneous score, so one bad month at the flagship doesn't crater everything overnight. It is read, live, by every Restaurant's own Reputation as a floor/boost — a new location opens with real starting buzz, an established one gets a small ongoing lift — the exact same "policy propagates, nothing cached" pattern already committed to for Suppliers and, per Phase 9, Brands: one company-level value, many live readers, no snapshots.

**And it is not a free lever — it plugs straight into the M5 prestige-erosion addendum rather than sitting beside it.** A name spread across forty doors buys each of them less than the same name on three, for the identical cookie-cutter reason erosion already models: personal fame spread that thin stops reading as personal. Chef Prestige should feed INTO that erosion formula as a second scale-sensitive input, not stay a separate, unbounded blessing — otherwise it quietly undoes the tension that addendum exists to create.

**Deliberately not built now.** Same gate as the rest of the M5 addendum below: this needs Reputation and Prestige to be real first (M3), and its actual payoff — propagating to a second location — needs Company/multi-location to exist at all (M4). Recorded here so the shape is captured, not to be started early.
...
Worth naming directly now that Reputation (Phase 4) formalizes it: "the founder's personal presence" above is **Personal Chef Prestige**, not a vague hand-wave — the exact value that thins out as it spreads across more doors, and the exact value the public-moments mechanic (award nominations, interviews — also added this session, see Reputation) is the way to actively renew rather than just passively hold.
...
**Reach is a property, and the hit is bounded by the truth (Aaron).** *"Even if an influencer says it, it shouldn't crater unless they are a big influencer — it should definitely have an impact but not everyone reads or follows everyone."* Correct, and the bound matters more than the scaling:

- **Reach varies, and big reach is rare.** Most influencers move standing slightly; a genuinely large one is itself an event. This falls out of systems that already exist — `ArchetypeProfile.LikelyAt` already sends influencers to the nightlife quarter, so nightlife sites naturally run more volatile in both directions. A location gains a personality beyond its traffic curve for free.
- **An influencer AMPLIFIES what is already true; it must not invent it.** The proposed shift is `reach x (their experience - current standing)`. A big name who eats a genuinely bad meal accelerates a decline the restaurant had coming; a big name who eats a genuinely good meal at an under-visited place delivers the best moment available in the game. A takedown of a genuinely good restaurant does little, because the gap it multiplies is small. **This is what keeps the event legible rather than a gotcha** — the outcome still traces to a decision the player made about their food, which Binding Principle 2 requires.
- **Allow a temporary overshoot that recovers over weeks.** A bad review should be a crisis to manage, not a permanent wound. Recovering it is gameplay; carrying it is punishment.
- **Design it as an OPPORTUNITY first.** Almost every consequence system built so far is a downside — price punishes, quality punishes, reputation decays. The rave from a big critic is the thing players will chase and retell, and Phase 8's design principle 6 (memorable moments) is better served by the upside than the takedown. If only one direction ships first, ship the good review.

**Deliberately NOT modelled:** real critics do sometimes damage genuinely good restaurants unfairly. That is true to life and wrong for this game — it is unexplainable bad luck, which the "hard and strategic, never unfair" principle rules out.
